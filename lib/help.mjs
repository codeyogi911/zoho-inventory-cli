...
export function showRootHelp(version, registry) {
  let out = `${CLI} ${version}\nCLI for the Zoho Inventory API (auto-refreshes OAuth tokens; honors ZOHO_INVENTORY_DC and ZOHO_INVENTORY_ORG_ID).\n\nUsage:\n  ${CLI} <resource> <action> [flags]\n  ${CLI} login [--refresh-token <t>] [--client-id <id>] [--client-secret <s>] [--status] [--clear]\n\nGlobal flags:\n  --json           Force JSON output\n  --dry-run        Print request without sending\n  --verbose        Print response status & headers to stderr\n  --all            Auto-paginate list actions (page/per_page)\n  --version, -v    Print version\n  --help, -h       Show this help\n\nResources:\n`;
  for (const r of Object.keys(registry).sort()) {
    out += `  ${r.padEnd(18)} ${Object.keys(registry[r]).join(", ")}\n`;
  }
  out += `\nCommands:\n  login              Store the OAuth refresh-token + client-id + client-secret (or a static access token).\n\nUse '${CLI} <resource> --help' for actions, or '<resource> <action> --help' for flags.\n`;
  return out;
}
